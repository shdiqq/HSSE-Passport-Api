...
namespace PTK.HSSEPassport.Api.Controllers.Masterdata
{
	...
	{
		private protected IPositionService _PositionRepo;

		public PositionController(IAppLogService appLog, IPositionService PositionRepo) : base(appLog)
		{
			_PositionRepo = PositionRepo;
		}

		[HttpPost("DataTable")]
		public async Task<IActionResult> DataTablePagination(PositionDTParamModel param, CancellationToken cancellationToken)
		{
			try
			{
				var result = await _PositionRepo.DataTablePagination(param, cancellationToken);
				return Ok(result);
			}
		...
		public async Task<IActionResult> GetById([FromRoute] int id, CancellationToken cancellationToken)
		{
			try
			{
				var result = await _PositionRepo.GetById(id, cancellationToken);
		...
		public async Task<IActionResult> GetAll(CancellationToken cancellationToken)
		{
			try
			{
				var result = await _PositionRepo.GetAll(cancellationToken);
				return Ok(new ReturnJson { Payload = result });
			}
			catch (DataNotFoundException e)
			{
		...
		[HttpPost]
		public async Task<IActionResult> Create(PositionModel param, CancellationToken cancellationToken)
		{
			try
			{
				var result = await _PositionRepo.Create(param, cancellationToken);
				return Ok(new ReturnJson { Payload = result });
			}
			catch (DomainLayerException e)
			{
		...
		[HttpPut("{id:int}")]
		public async Task<IActionResult> Update([FromRoute] int id, PositionModel param, CancellationToken cancellationToken)
		{
			try
			{
				var result = await _PositionRepo.Update(id, param, cancellationToken);
				return Ok(new ReturnJson { Payload = result });
			}
			catch (DataNotFoundException e)
			{
		...
		public async Task<IActionResult> Delete([FromRoute] int id, [FromQuery] string updatedBy, CancellationToken cancellationToken)
		{
			try
			{
				var result = await _PositionRepo.Delete(id, updatedBy, cancellationToken);
				return Ok(new ReturnJson { Payload = result });
			}
			catch (DataNotFoundException e)
			{
